...
8. **Justificación técnica de cada archivo adjunto:**
   - `forge614-shell-1.0.0.tar.gz`: Archivo comprimido con el runtime transpilado y optimizado de Node.js. Permite al colaborador instalar y ejecutar Forge614 Shell de forma autónoma sin clonar el repositorio de Git ni descargar dependencias de desarrollo (`node_modules` o Bun).
   - `forge614-shell-1.0.0.tar.gz.sha256`: Sello criptográfico que permite a los colaboradores (y a scripts automatizados) verificar la integridad del paquete descargado antes de ejecutarlo, garantizando que el archivo no sufra alteraciones ni corrupción durante la descarga.
   - `install.sh`: Script auxiliar de instalación que automatiza la descompresión segura en dos fases, el despliegue versionado en `~/.forge614/shell/1.0.0/` y la actualización atómica del enlace simbólico en `~/.forge614/bin/forge614-shell`.
