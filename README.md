...
**Status:** Stage 02 in progress (Basic split interface with contextual sidebar, braille context ring, bottom status bar with project identity and pinned `v1.0.0` version, native work modes via `Shift+Tab`, Codex skill discovery via `$`, `/refresh` quota queries, independent scrolling, layered architecture, multi-engine adapters, shared profiles, local `/logout`, `/login` reconnect, 1.0.0 private release published to GitHub with 3 downloadable assets, installer with automatic PATH injection requiring zero manual steps, and immutable 1.0.1 patch policy; 124 passing tests across 32 files). Stage 2 is not completed; Full Orca-style workspace manager, unattended remote release distribution, Windows installer, local inference engines, and Engram memory integration remain deferred.
